...
async def save_generated_files_locally(task_id: str, files: dict) -> str:
    ...
    for filename, content in files.items():
        ...
        try:
            # Step 1: decode escaped \n, \t, etc.
            decoded_content = content.encode("utf-8").decode("unicode_escape")

            # Step 2: convert HTML entities like &lt; &gt; &amp;
            decoded_content = html.unescape(decoded_content)

            with open(file_path, "w", encoding="utf-8") as f:
                f.write(decoded_content)

            logger.info(f"   -> Saved: {filename} (bytes: {len(decoded_content)})")
        except Exception as e:
...
async def receive_task(task_data: TaskRequest, request: Request):
    ...
    if not verify_secret(task_data.secret):
        logger.warning(
            f"Unauthorized attempt for task {task_data.task} "
    ...
    logger.info(f"Received task {task_data.task}. Starting full process synchronously...")
    flush_logs()

    try:
        # Wait for full generation and deployment to complete
        await task_semaphore.acquire()
        try:
            await generate_files_and_deploy(task_data)
        finally:
            task_semaphore.release()

        # Build repo/page URLs
        ...
        repo_path = os.path.join(os.getcwd(), "generated_tasks", task_data.task)
        try:
        ...
        logger.info(f"[READY_RESPONSE] Returning full deployment details for {task_data.task}")
        flush_logs()
        return JSONResponse(status_code=200, content=response_content)

    except Exception as e:
        logger.exception(f"[READY_RESPONSE] Error processing task {task_data.task}: {e}")
        flush_logs()
